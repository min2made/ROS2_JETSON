...
# 환경 팩토리
# ──────────────────────────────────────────────────────────────
def make_env(node: RobotSensorNode):
    """Monitor 감싸기로 에피소드 통계 자동 기록."""
    def _init():
        env = GazeboRoomEnv(node=node)
...
def main():
    ...
    # ── 환경 구성 ──────────────────────────────────────────
    env = DummyVecEnv([make_env(sensor_node)])

    # VecNormalize: reward 정규화 (lidar는 이미 [0,1] 정규화이므로 obs는 제외)
    ...
    if args.resume:
        ...
        if ckpt is None:
            ...
            if ckpt_files:
                ckpt = os.path.join(MODEL_DIR, ckpt_files[-1])
    ...
    checkpoint_cb = CheckpointCallback(
        save_freq     = 20_000,
        save_path     = MODEL_DIR,
        name_prefix   = 'ppo_bookstore',
        save_vecnormalize = True,
    )

    eval_cb = EvalCallback(
        eval_env           = DummyVecEnv([make_env(sensor_node)]),
    ...
    # ── 학습 시작 ──────────────────────────────────────────
    print(f'[train_rl] PPO 학습 시작 — 총 {args.timesteps:,} 스텝')
    ...
    # ── 최종 저장 ──────────────────────────────────────────
    final_path = os.path.join(MODEL_DIR, 'ppo_bookstore_final')
